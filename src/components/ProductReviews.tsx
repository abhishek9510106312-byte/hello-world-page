...
import { Star, User, Loader2, Trash2, Pencil, X, Check, BadgeCheck } from "lucide-react";
...
const ProductReviews = ({ productId }: ProductReviewsProps) => {
  ...
  // Fetch reviews with user profiles and verified purchase status
  const { data: reviews = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (userIds.length > 0) {
        ...
        // Check verified purchases for all reviewers
        ...
        // Create set of verified purchaser user_ids
        const verifiedPurchasers = new Set(
          purchaseData?.map(item => (item.orders as any)?.user_id).filter(Boolean) || []
        );
        ...
        return reviewsData.map(review => ({
          ...
          isVerifiedPurchase: verifiedPurchasers.has(review.user_id),
        })) as Review[];
      }
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {review.isVerifiedPurchase && (
                              <span className="inline-flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 px-2 py-0.5 rounded-full">
